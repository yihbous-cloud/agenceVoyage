...
export function buildReceiptPdfBuffer({ agency, payment }) {
  return new Promise((resolve, reject) => {
    ...
    // Un paiement de service visa autonome n'est rattaché à aucun voyage.
    if (payment.isVisaService) {
      drawField("Service", `${payment.visa_type_name}${payment.country ? ` (${payment.country})` : ""}`);
    } else {
    ...
    let dueLabel = "Montant total du voyage";
    if (payment.members) dueLabel = "Montant dû (groupe)";
    else if (payment.isVisaService) dueLabel = "Montant dû (visa)";
    drawField(dueLabel, formatAmount(totalDue, payment.currency));
