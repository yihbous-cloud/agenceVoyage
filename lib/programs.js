...
// proche (nt = "nearest trip") pour les champs affichés sur les cartes (dates,
// aéroport de départ, distance au point de repère, places restantes...), mais
// le prix affiché (starting_price) est le plus bas de TOUS les voyages ouverts
// du programme (pas seulement le plus proche) — cf. demande explicite de
// l'utilisateur, voir CLAUDE.md.
//
// filters:
...
export async function getProgramsByFamily(family, filters = {}) {
  ...
  // Même filtre destination, appliqué à la sous-requête de prix minimum du
  // programme (t2, distincte de la sous-requête "voyage le plus proche" ci-
  // dessus) — un programme filtré par destination doit afficher le prix le
  // plus bas parmi les voyages qui correspondent à ce filtre, pas parmi tous.
  let minPriceFilterExtra = "";
  const minPriceParams = [];
  if (family === "voyage_organise" && filters.destination) {
    minPriceFilterExtra = " AND t2.destination_country LIKE ?";
    minPriceParams.push(`%${filters.destination}%`);
  }
  ...
  const params = [...minPriceParams, ...nearestTripParams, ...whereParams];
  ...
  return query(
    `SELECT p.id, p.title, p.slug, p.program_type, p.family, p.season, p.theme,
            ...
            (
              SELECT MIN(
                LEAST(
                  t2.price_double, t2.price_triple, t2.price_quadruple, t2.price_quintuple,
                  COALESCE(tier_min.min_price, t2.price_double)
                )
              )
              FROM trips t2
              LEFT JOIN (
                SELECT tht.trip_id, MIN(thtp.price_per_person) AS min_price
                FROM trip_hotel_tiers tht
                JOIN trip_hotel_tier_prices thtp ON thtp.tier_id = tht.id
                GROUP BY tht.trip_id
              ) tier_min ON tier_min.trip_id = t2.id
              WHERE t2.program_id = p.id
                AND t2.status IN ('ouvert', 'planifie')
                AND t2.departure_date >= CURDATE()
                ${minPriceFilterExtra}
            ) AS starting_price,
            nt.currency,
