...
export async function createHotel(data) {
  const result = await query(
    `INSERT INTO hotels (name, city, country, star_rating, distance_to_haram_m, contact_info)
     VALUES (?, ?, ?, ?, ?, ?)`,
    [
      data.name,
      data.city,
...
export async function updateHotel(id, data) {
  await query(
    `UPDATE hotels SET name = ?, city = ?, country = ?, star_rating = ?, distance_to_haram_m = ?, contact_info = ?
     WHERE id = ?`,
    [
      data.name,
      data.city,
