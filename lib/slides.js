...
export async function createSlide(data) {
  ...
  const result = await query(
    `INSERT INTO slides (title, subtitle, image_url, button_text, program_id, button_link, sort_order, is_active)
     VALUES (?, ?, ?, ?, ?, ?, ?, ?)`,
    [
      data.title,
      data.subtitle || null,
...
export async function updateSlide(id, data) {
  await query(
    `UPDATE slides SET title = ?, subtitle = ?, image_url = ?, button_text = ?, program_id = ?, button_link = ?, is_active = ?
     WHERE id = ?`,
    [
      data.title,
      data.subtitle || null,
