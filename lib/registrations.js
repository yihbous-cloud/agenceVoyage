...
export async function listRegistrations({ tripId, status } = {}) {
  const conditions = [];
  const params = [];
...
export async function getRegistrationById(id) {
  const rows = await query(
    `SELECT r.*, tr.full_name, tr.full_name_arabic, tr.gender, tr.date_of_birth,
            ...
            ph.name AS preferred_hotel_name,
            rg.label AS group_label, rg.allow_mixed_gender_room
     FROM registrations r
     JOIN travelers tr ON tr.id = r.traveler_id
     JOIN trips t ON t.id = r.trip_id
     JOIN programs p ON p.id = t.program_id
     LEFT JOIN hotels ph ON ph.id = r.preferred_hotel_id
...
export async function createRegistration(data) {
  ...
  try {
    ...
    const [registrationResult] = await connection.execute(
      `INSERT INTO registrations
         (trip_id, traveler_id, registered_by_staff_id, status, total_due, notes,
          preferred_hotel_id, preferred_room_type, group_id)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`,
      [
        data.tripId,
        travelerId,
    ...
    return { id: registrationResult.insertId, travelerId };
  } catch (err) {
...
export async function updateRegistration(id, data) {
  ...
  const map = {
    ...
    roomId: "room_id",
    notes: "notes",
    preferredHotelId: "preferred_hotel_id",
