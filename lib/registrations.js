...
export async function getRegistrationById(id) {
  const rows = await query(
    `SELECT r.*, tr.full_name, tr.full_name_arabic, tr.gender, tr.date_of_birth,
            tr.national_id, tr.passport_number, tr.passport_expiry_date,
...
export async function updateTraveler(travelerId, data) {
  await query(
    `UPDATE travelers SET full_name = ?, phone_whatsapp = ?, gender = ?, passport_number = ?, passport_expiry_date = ?, email = ?
     WHERE id = ?`,
    [
      data.fullName,
      data.phoneWhatsapp,
