...
export async function createRegistration(data) {
  ...
  try {
    ...
    // demandé (ou le prix le plus bas si aucune préférence exprimée, voir
    // CLAUDE.md), pour ne pas partir de 0 et obliger le personnel à le
    // ressaisir — reste modifiable ensuite (EditRegistrationForm.jsx). Un
    // appelant peut toujours forcer un autre montant via data.totalDue
    // (ex. tarif négocié).
    let totalDue = data.totalDue;
    if (totalDue === undefined || totalDue === null || totalDue === "") {
      const [[trip]] = await connection.execute(
        `SELECT price_double, price_triple, price_quadruple, price_quintuple FROM trips WHERE id = ?`,
        [data.tripId]
      );
...
export async function listOpenTripsForSelect() {
  return query(
    `SELECT t.id, t.reference_code, t.departure_date,
