...
export async function getTripSummary(tripId) {
  const rows = await query(
    `SELECT t.id, t.program_id, t.reference_code, t.departure_date, t.return_date,
            p.title AS program_title, p.family AS program_family
     FROM trips t
...
// Un voyageur ne peut pas être dans deux hôtels en même temps, même dans des
// villes différentes (ex. escale-séjour à Istanbul avant l'Arabie Saoudite,
// voir CLAUDE.md) — détecte tout hôtel du voyage dont la période chevauche
// celle proposée. Intervalles semi-ouverts : un check-out le même jour que
// le check-in suivant est autorisé (jour de transition normal), pas un
// chevauchement.
export async function findOverlappingTripHotel(tripId, checkInDate, checkOutDate) {
  const rows = await query(
...
export async function listUnassignedRegistrations(tripId) {
  const registrations = await query(
    `SELECT r.id, r.room_id, r.preferred_room_type,
     ...
     WHERE r.trip_id = ? AND r.room_id IS NULL
       AND (
         ...
         OR (r.group_id IS NOT NULL AND EXISTS (
           SELECT 1 FROM payments p WHERE p.group_id = r.group_id
         ))
       )
  ...
  return registrations.map((r) => ({
    ...r,
    hotelPreferences: preferencesByRegistration.get(r.id) || [],
  }));
}
...
export async function listAssignedRegistrationsForTrip(tripId) {
  return query(
    `SELECT r.id, r.room_id, r.preferred_room_type,
            tr.full_name, tr.gender,
            rhp.hotel_id AS preferred_hotel_id, pref_hotel.name AS preferred_hotel_name
     FROM registrations r
     JOIN travelers tr ON tr.id = r.traveler_id
     JOIN rooms rm ON rm.id = r.room_id
     JOIN trip_hotels th ON th.id = rm.trip_hotel_id
     JOIN hotels room_hotel ON room_hotel.id = th.hotel_id
     LEFT JOIN registration_hotel_preferences rhp
       ON rhp.registration_id = r.id AND rhp.city = room_hotel.city
     LEFT JOIN hotels pref_hotel ON pref_hotel.id = rhp.hotel_id
...
export async function autoAssignTrip(tripId) {
  ...
  const hasPreference = (reg) =>
    Boolean((reg.hotelPreferences && reg.hotelPreferences.length > 0) || reg.preferred_room_type);
  ...
  for (const gender of genderOrder) {
    for (const reg of byGender[gender]) {
      ...
      const preferenceScore = (r) =>
        (reg.hotelPreferences?.some((p) => p.hotel_id === r.hotelId) ? 2 : 0) +
