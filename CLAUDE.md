...
- Billet d'avion : ~~prix rattaché au **voyage précis** (`trips.flight_ticket_price`)~~ — **retiré** depuis §3novovicies, désormais inclus dans `trips.price_per_person` comme le visa et les autres services (colonne conservée en base, dépréciée, plus alimentée)
...
- **Pays de destination (premier voyage) en zone de texte déroulante** : `<input list="trip-destination-countries">` + `<datalist>` sur `COUNTRIES` (`lib/worldPlaces.js`, même référence que Pays/Ville des hôtels, §3octies) — saisie libre toujours possible, filtrage natif du navigateur en tapant. Valeur par défaut inchangée : "Arabie Saoudite"
