...
- `hotels.reserved_rooms_simple/double/triple/quadruple/quintuple` (`INT UNSIGNED DEFAULT 0` chacun, un par valeur de `ROOM_TYPES` — y compris `simple`, contrairement aux formulaires de préférence/prix qui l'excluent, §3unetrigies : un quota de blocs réservés n'a pas la même contrainte, un hôtel peut très bien avoir des chambres simples bloquées pour l'agence)
