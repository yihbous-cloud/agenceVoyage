        ...
        .map((t) => (
            <div key={t.id} className="space-y-3 rounded-xl border border-zinc-200 bg-zinc-50 p-4">
              <p className="text-sm text-zinc-500">
                <Link
                  href={`/admin/voyages/${t.id}/hebergement`}
                  className="text-emerald-700 hover:underline"
                >
                  Hébergement
                </Link>
                {" · "}
                <Link
                  href={`/admin/voyages/${t.id}/listes`}
                  className="text-emerald-700 hover:underline"
                >
                  Listes
                </Link>
                {" · "}
                <Link
                  href={`/admin/voyages/${t.id}/billets`}
                  className="text-emerald-700 hover:underline"
                >
                  Billets d&apos;avion
                </Link>
                {" · "}
                <Link
                  href={`/admin/inscriptions?tripId=${t.id}`}
                  className="text-emerald-700 hover:underline"
                >
                  Inscrits
                </Link>
              </p>
              <TripForm trip={t} airlines={airlines} canDelete={canManage} />
            </div>
          ))}
    </div>
  );
